...
class CTkTable(customtkinter.CTkScrollableFrame):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        super().__init__(master, width = withFrame, height = heightFrame)
        ...
        self.write = write
        self.api = SecurityAnalysisToolkit()
    ...
    def action_window(self, host):
        if hasattr(self, "_newWindow") and self._newWindow.winfo_exists():
            # If the window already exists, close it
            self._newWindow.destroy()
        
        # Create a new window
        self._newWindow = customtkinter.CTkToplevel(self)
        self._newWindow.title(host)
        ...
        self._newWindow.geometry("500x250")  # Width x Height

        # Position the window in the center of the screen
        window_width = self._newWindow.winfo_reqwidth()
        window_height = self._newWindow.winfo_reqheight()
        position_right = int(self._newWindow.winfo_screenwidth()/2 - window_width/2)
    ...
    def open_file_with_default_app(self, file_path):
        ...
        try:
            if sys.platform.startswith('linux'):
                ...
                subprocess.run(["cmd", "/c", "start", "", full_path], shell=True)
            else:
            ...
            for j in range(self.columns):
                ...
                if self.write:
                    ...
                    if "hover_color" not in args:
                        args["hover_color"] = self.hover_color #customtkinter.ThemeManager.theme["CTkButton"]["hover_color"]#self.hover_color
                    ...
                    self.frame[i,j] = customtkinter.CTkButton(self.inside_frame, background_corner_colors=corners,
                                                              ...
                                                              text=value,
                                                              border_width=0,
                                                              command=(lambda value=self.data[i,j]['value']: self.action_window(value)) if self.command and i!=0 and j==0 else None, 
                                                              **args
                                                              )
                    
                    if self.data[i,j]["column"] != 0 or self.data[i,j]["row"] == 0: 
                        self.frame[i,j].configure(hover = None, hover_color= customtkinter.ThemeManager.theme["CTkButton"]["hover_color"])
